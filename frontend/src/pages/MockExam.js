...
export default function MockExamPage({ profile }) {
  ...
  const examForTimer = status?.exam;

  useEffect(() => {
    if (!examForTimer || examForTimer.status !== 'active') return undefined;
    const timer = setInterval(() => setTimerTick((prev) => prev + 1), 1000);
    return () => clearInterval(timer);
  }, [examForTimer, examForTimer?.exam_id, examForTimer?.status]);
  ...
  const remainingSeconds = useMemo(() => {
    if (!activeExam) return 0;
    return Math.max(0, activeExam.duration_minutes * 60 - elapsedSeconds);
  }, [activeExam, elapsedSeconds]);
    ...
    return (
      <div className="page-shell fade-in">
        <div className="state-card loading-shimmer">
          <div className="state-title">Загрузка пробника</div>
          <div className="state-description">Подтягиваем режимы, активный экзамен и последние завершённые попытки.</div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="page-shell page-shell-wide fade-in">
        <div className="hero-card dashboard-hero">
          ...
          <div className="hero-description">Пробник собирается из случайных заданий банка. Навигация свободная, проверка только после завершения.</div>

          <div style={{ marginTop: 18 }} className="segment-control">
            {(status?.modes || []).map((mode) => (
              <button key={mode.mode} className={`segment-btn ${selectedMode === mode.mode ? 'active' : ''}`} onClick={() => setSelectedMode(mode.mode)}>{mode.label}</button>
            ))}
          </div>

          <div className="analytics-grid" style={{ marginTop: 18 }}>
            {(status?.modes || []).map((mode) => (
              <div key={mode.mode} className="metric-card">
                <div className="metric-value" style={{ color: mode.mode === 'exam' ? 'var(--accent-strong)' : 'var(--warning)' }}>{mode.tasks_count}</div>
                <div className="metric-label">{mode.label}</div>
                <div style={{ marginTop: 10, fontSize: 13, lineHeight: 1.65, color: 'var(--text-secondary)' }}>{mode.description}</div>
                <div className="task-chip-row" style={{ marginTop: 12 }}><span className="badge badge-muted">{mode.duration_minutes} мин.</span></div>
              </div>
            ))}
          </div>

          <div className="row-actions" style={{ marginTop: 20 }}>
            <button className="btn btn-primary btn-lg" onClick={startExam}>Начать {selectedMode === 'exam' ? 'экзаменационный' : 'тренировочный'} режим</button>
          </div>
        </div>

        {latestCompleted ? (
          <div className="card">
            <div className="section-title">Последний завершённый пробник</div>
            <div className="info-grid" style={{ marginTop: 18 }}>
              <div className="info-card"><div className="info-label">Режим</div><div className="info-value">{latestCompleted.mode === 'training' ? 'Тренировочный' : 'Экзаменационный'}</div></div>
              <div className="info-card"><div className="info-label">Результат</div><div className="info-value">{latestCompleted.score?.toFixed?.(0) || 0}%</div></div>
              <div className="info-card"><div className="info-label">Дата</div><div className="info-value">{latestCompleted.completed_at?.slice?.(0, 10) || '—'}</div></div>
            </div>
          </div>
        ) : null}
      </div>
    ...
    return (
      <div className="page-shell fade-in">
        <div className="hero-card dashboard-hero">
          ...
          <div className="hero-description">Итог уже учтён в статистике. Ниже доступны слабые номера и переход к их доработке.</div>

          <div className="row-actions" style={{ marginTop: 16 }}>
            <div className="timer-chip">{formatTime(submittedResult.elapsed_seconds || 0)}</div>
            <span className="badge badge-accent">{submittedResult.mode === 'training' ? 'Тренировочный' : 'Экзаменационный'}</span>
          </div>
        ...
        {Array.isArray(submittedResult.weak_tasks) && submittedResult.weak_tasks.length > 0 ? (
          <div className="card">
            <div className="section-title">Слабые номера после пробника</div>
            <div className="task-chip-row" style={{ marginTop: 14 }}>
              {submittedResult.weak_tasks.map((item) => (
                <div key={`weak-${item.task_number}`} className="card-elevated" style={{ minWidth: 220 }}>
                  <div style={{ fontSize: 15, fontWeight: 740 }}>№{item.task_number}</div>
                  <div style={{ marginTop: 8, fontSize: 12, color: 'var(--text-secondary)' }}>{item.reason === 'exercise_missing' ? 'Нет задания в локальной базе' : 'Нужна доработка после ошибки'}</div>
                  <div className="row-actions" style={{ marginTop: 10 }}>
                    <Link className="btn btn-ghost btn-sm" to={item.theory_link || `/theory?task=${item.task_number}`}>Теория</Link>
                    <Link className="btn btn-secondary btn-sm" to={item.practice_link || `/practice?task=${item.task_number}&mode=weak`}>Практика</Link>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="table-like-list" style={{ marginTop: 16 }}>
            {(submittedResult.results || []).map((result) => (
              <div key={result.task_number} className="table-like-row">
                ...
                <div>
                  <div style={{ fontSize: 14, fontWeight: 740 }}>{result.correct ? 'Верно' : 'Нужно разобрать'}</div>
                  <div style={{ marginTop: 6, fontSize: 12, lineHeight: 1.65, color: 'var(--text-secondary)' }}>{result.explanation || result.details || 'Без дополнительного комментария.'}</div>
                </div>
                <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6, flexWrap: 'wrap' }}>
                  <Link className="btn btn-ghost btn-sm" to={result.theory_link || `/theory?task=${result.task_number}`}>Теория</Link>
                  <Link className="btn btn-ghost btn-sm" to={result.practice_link || `/practice?task=${result.task_number}&mode=${result.correct ? 'free' : 'weak'}`}>Практика</Link>
                </div>
                <div />
              </div>
            ))}
    ...
    <div className="page-shell page-shell-wide fade-in">
      <div className="mock-layout">
        <aside className="left-rail-card">
          ...
          <div className="card">
            <div className="section-title" style={{ fontSize: 18 }}>Навигация по задачам</div>
            <div className="mock-nav-grid" style={{ marginTop: 14 }}>
        ...
        <section className="page-stack">
          {activeExam.status === 'paused' ? (
            <div className="status-banner warning">
              <div style={{ fontWeight: 780, fontSize: 15 }}>Пробник поставлен на паузу</div>
              <div style={{ marginTop: 8, fontSize: 13, lineHeight: 1.65, color: 'var(--text-secondary)' }}>Таймер остановлен. Нажмите «Продолжить», чтобы вернуться к решению.</div>
            </div>
          ) : null}
          ...
          {currentTask ? (
            <div className="card">
              ...
              <div style={{ marginTop: 12, fontSize: 14, lineHeight: 1.78, whiteSpace: 'pre-wrap', color: 'var(--text-secondary)' }}>
                {currentTask.exercise?.text || 'Для этой задачи не найдено локальное упражнение.'}
              </div>
              ...
              {currentTask.exercise?.exercise_type === 'code' ? (
                ...
                <div className="choice-list" style={{ marginTop: 20 }}>
                  {currentTask.exercise.options.map((option, index) => {
                    ...
                    return <ChoiceButton key={index} active={active} leading={value} label={option} onClick={() => {
                ...
                <div className="choice-list" style={{ marginTop: 20 }}>
                  {currentTask.exercise.options.map((option, index) => {
                    ...
                    return <ChoiceButton key={index} active={active} leading={active ? '✓' : '+'} label={option} onClick={() => {
